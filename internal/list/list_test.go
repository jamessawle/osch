...
func TestListEmptyDir(t *testing.T) {
	...
	if got := strings.TrimSpace(buf.String()); got != "no schemas installed" {
		t.Errorf("unexpected output: %q", got)
	}
}

func TestListSchemasDirExistsButEmpty(t *testing.T) {
	dir := t.TempDir()
	...
	out := buf.String()
	if !strings.Contains(out, "NAME") || !strings.Contains(out, "ACTIVE") || !strings.Contains(out, "TRACKED") {
...
func TestListOneTrackedActive(t *testing.T) {
	...
	writeFile(t, filepath.Join(dir, "openspec", "config.yml"), "schema: widget\n")
...
func TestListOneTrackedInactive(t *testing.T) {
	dir := t.TempDir()
	mkSchema(t, dir, "widget", true)
	writeFile(t, filepath.Join(dir, "openspec", "config.yml"), "schema: gadget\n")
...
func TestListMix(t *testing.T) {
	...
	writeFile(t, filepath.Join(dir, "openspec", "config.yml"), "schema: gamma\n")
	...
	// rows should be sorted by name (alpha, beta, gamma) after the header.
	lines := strings.Split(strings.TrimRight(out, "\n"), "\n")
	if len(lines) != 4 {
...
func TestListConfigWithoutSchemaKey(t *testing.T) {
	dir := t.TempDir()
	mkSchema(t, dir, "widget", true)
	writeFile(t, filepath.Join(dir, "openspec", "config.yml"), "other: thing\n")
...
func schemaRow(t *testing.T, out, name string) [3]string {
	...
	for _, line := range strings.Split(out, "\n") {
		...
		case 2:
			// ACTIVE column collapsed to blank: NAME then TRACKED.
